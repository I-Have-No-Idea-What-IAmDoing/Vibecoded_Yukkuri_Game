...
from ...engine.events import InventoryChangedEvent, EntityDestroyedEvent
    ...
    def _handle_pickups(self, world: World) -> None:
        ...
        for entity_id, (inventory, request) in world.get_components_tuple(
            ...
            if inventory.can_add(item_type_id, count=1, stack_limit=stack_limit):
                added_count = inventory.add(item_type_id, count=1, stack_limit=stack_limit)
                ...
                if added_count > 0:
                    ...
                    logger.info(f"Entity {entity_id} picked up item {item_type_id} (Entity {target_id}).")
                else:
                    logger.debug(f"Entity {entity_id} could not add item {item_type_id} (Inventory full?)")
            else:
                 logger.debug(f"Inventory full for {entity_id}, cannot pickup {item_type_id}.")
        ...
        for entity_id, (inventory, transform, request) in world.get_components_tuple(
            ...
            if inventory.has(item_type_id, count):
                ...
                if removed > 0:
                    ...
                    spawn_x = transform.x
                    spawn_y = transform.y + 20 # Slight offset so it doesn't clip usually
                    ...
                    try:
                        ...
                        logger.info(f"Entity {entity_id} dropped {removed}x {item_type_id}.")
                        
                    except ValueError as e:
                         # Item type might not exist in factory logic
                         logger.error(f"Failed to spawn dropped item {item_type_id}: {e}")
                         # Refund item?
                         inventory.add(item_type_id, removed, 999) # Force add back
                
            else:
                 logger.debug(f"Entity {entity_id} tried to drop {count}x {item_type_id} but didn't have enough.")
