...
class SoftwareLightingEngine:
        ...
        if occluders:
            if soft_shadows:
                self._draw_soft_shadows(
                    light_surf, sr_key, lx, ly, sx, sy, radius, occluders
                )
            elif HAS_NUMPY:
    ...
    def _draw_shadow_volumes_numpy(
        ...
        for _, vertices in occluders:
            ...
            q2 = ((p2 + ex2) * scale - np.array([sx_scaled, sy_scaled])) + np.array(
                [hx, hy]
            )
            q3 = ((p1 + ex1) * scale - np.array([sx_scaled, sy_scaled])) + np.array(
                [hx, hy]
            )
