...
class HudLayout:
    ...
    def create_settings_window(self, current_settings: Any) -> None:
        """
        Creates the settings window.

        Args:
            current_settings (UserSettings): Current settings values.
        """
        ...
        # Use attribute access or .get depending on if it's a dict or Struct
        # To support both (e.g. tests using dict), check type or try/except
        if isinstance(current_settings, dict):
             ...
             if isinstance(audio_settings, dict):
                 ...
                 master_vol = getattr(audio_settings, "master_volume", 1.0) * 100
                 bgm_vol = getattr(audio_settings, "bgm_volume", 1.0) * 100
                 sfx_vol = getattr(audio_settings, "sfx_volume", 1.0) * 100
             ...
             if isinstance(window_settings, dict):
                 ...
                 width = getattr(window_settings, "width", 1280)
                 height = getattr(window_settings, "height", 720)
                 fullscreen = getattr(window_settings, "fullscreen", False)
        else:
             # Assume UserSettings Struct
             audio_settings = getattr(current_settings, "audio", None)
             if audio_settings:
                 master_vol = audio_settings.master_volume * 100
                 bgm_vol = audio_settings.bgm_volume * 100
                 sfx_vol = audio_settings.sfx_volume * 100
             else:
                 master_vol = 100
                 bgm_vol = 100
                 sfx_vol = 100

             window_settings = getattr(current_settings, "window", None)
             if window_settings:
