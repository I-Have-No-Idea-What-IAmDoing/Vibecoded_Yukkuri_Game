...
class SkillService:
    ...
    def __init__(self, world: World):
        ...
        self.data_dir = os.path.join("data", "skills")
        self.load_skills_data()

    def load_skills_data(self) -> None:
        """Loads skill definitions from TOML."""
        filepath = os.path.join(self.data_dir, "skills.toml")
        if not os.path.exists(filepath):
            logger.warning(f"Skills data file not found: {filepath}")
            return
    ...
    def initialize_skills(self, entity: int) -> None:
        ...
        for skill_id, skill_def in self.skills_data.items():
            state = SkillState()
    ...
    def add_xp(self, entity: int, skill_id: str, amount: float) -> None:
        ...
        skill_def = self.skills_data.get(skill_id, {})
        ...
        # Soft Cap Logic
        soft_cap_base = skill_def.get("soft_cap_base_level", 10)
        ...
        while state.current_xp >= required:
            max_lvl = skill_def.get("max_level", 20)
    ...
    def apply_decay(self, entity: int, current_gametime: float) -> None:
        ...
        for skill_id, state in skills_comp.states.items():
            skill_def = self.skills_data.get(skill_id, {})
            decay_rate = skill_def.get("decay_rate", 0.0)
