...
class UtilityAI:
    ...
    def score_action(self, action_name: str, action_def: Dict, entity: Entity, entities: List[Entity]) -> float:
        ...
        # Utility Calculation: typically multiplicative or averaged.
        # We will use multiplicative modification factor (0-1) averaged

        total_mod = 1.0
        # A more robust way is averaging the considerations or using a weighted sum.
        # Let's use a weighted sum normalized to 0-1 range then multiply by importance?
        # Simpler approach for MVP: Average of considerations (0-1).
