...
pub const FLIGHT_STATE_GROUNDED: u32 = 0;
pub const FLIGHT_STATE_TAKEOFF: u32 = 1;
pub const FLIGHT_STATE_FLYING: u32 = 2;
pub const FLIGHT_STATE_HOVERING: u32 = 3;
pub const FLIGHT_STATE_LANDING: u32 = 4;
pub const FLIGHT_STATE_SWOOPING: u32 = 5;
pub const FLIGHT_STATE_FALLING: u32 = 6;
...
pub fn flight_system(
    ...
    for (entity, mut flight, maybe_skills, mut maybe_needs, mut maybe_emotional) in query.iter_mut() {
        ...
        if flight.flight_state == FLIGHT_STATE_FLYING || flight.flight_state == FLIGHT_STATE_TAKEOFF || flight.flight_state == FLIGHT_STATE_SWOOPING {
        ...
        } else if flight.flight_state == FLIGHT_STATE_HOVERING {
            // HOVERING
        ...
        } else if flight.flight_state == FLIGHT_STATE_GROUNDED {
            // GROUNDED
        ...
        if flight.stamina <= 0.0 && flight.flight_state != FLIGHT_STATE_GROUNDED && flight.flight_state != FLIGHT_STATE_FALLING {
            flight.flight_state = FLIGHT_STATE_FALLING; // Set state to FALLING
        }
        ...
        match flight.flight_state {
            FLIGHT_STATE_GROUNDED => target_altitude = 0.0, // GROUNDED
            FLIGHT_STATE_TAKEOFF => { // TAKEOFF
                target_altitude = flight.max_altitude;
                if flight.altitude >= flight.max_altitude * 0.95 {
                    flight.flight_state = FLIGHT_STATE_FLYING; // Transitions to FLYING
                }
            }
            FLIGHT_STATE_FLYING | FLIGHT_STATE_HOVERING => target_altitude = flight.max_altitude, // FLYING or HOVERING
            FLIGHT_STATE_LANDING => { // LANDING
                target_altitude = 0.0;
                if flight.altitude <= 0.1 {
                    flight.flight_state = FLIGHT_STATE_GROUNDED; // Transitions to GROUNDED
                    flight.altitude = 0.0;
                }
            }
            FLIGHT_STATE_SWOOPING => target_altitude = 5.0, // SWOOPING target altitude
            FLIGHT_STATE_FALLING => { // FALLING
...
pub fn flight_collision_modulation_system(
    ...
    for (entity, flight) in query.iter() {
        let new_layers = if flight.flight_state == FLIGHT_STATE_FLYING || flight.flight_state == FLIGHT_STATE_HOVERING {
        ...
        } else if flight.flight_state == FLIGHT_STATE_SWOOPING {
