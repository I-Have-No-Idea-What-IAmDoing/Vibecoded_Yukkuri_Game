...
from yukkuri_game.game.settings_service import SettingsService, UserSettings, AudioSettings, WindowSettings
...
class TestSettingsIntegration(unittest.TestCase):
    ...
    def test_settings_service_load_save(self) -> None:
        ...
        # Check content
        with open(self.test_settings_file, "rb") as f:
            content = msgspec.toml.decode(f.read(), type=UserSettings)
            self.assertEqual(content.audio.master_volume, 0.8)
