...
from yukkuri_game.game.components import Transform, PhysicsBody, MovementController, SteeringComponent, Vision
...
class TestPredatorMovingTarget(unittest.TestCase):
    def setUp(self):
        ...
        self.nav_service = NavServiceClass(world_width=1000, world_height=1000, grid_step_size=25)
    ...
    def test_chase_moving_target(self):
        try:
            ...
            predator_ai.visible_entities.add(prey_id)  # Make prey visible for Close-Range + LKP
            ...
            # 4. Behavior
            move_action = MoveToTarget(entity_id=predator_id, world=self.world, speed=100.0)
            ...
            self.assertLess(min_dist, initial_dist * 0.5, f"Predator never closed gap. Initial: {initial_dist}, Min: {min_dist}")
            self.assertLess(min_dist, 100.0, f"Predator never got close enough. Min dist: {min_dist}")
