...
class TestAdaptiveDrift(unittest.TestCase):
    ...
    def test_adaptive_threshold(self):
        try:
            ...
            target_trans.x = 140
            action.update()
            self.assertIsNotNone(ai.path, "Slow target, drift 40px should not trigger repath (Threshold 50px)")
            ...
            action.update()
            self.assertIsNone(ai.path, "Fast target, drift 30px should trigger repath (Threshold ~20px)")
