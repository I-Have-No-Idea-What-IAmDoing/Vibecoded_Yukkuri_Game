...
from yukkuri_game.game.ai.navigation_service import NavigationService, TraversalCapability
...
class TestNavigationIntegration(unittest.TestCase):
    def setUp(self):
        ...
        self.nav_service = NavigationService(world_width=1000, world_height=1000, grid_step_size=25)
    ...
    def test_navigation_flow_with_obstacle(self):
        ...
        yukkuri_id = self.world.create_entity(
        ...
        wall_id = self.world.create_entity(
            PhysicsBody(body=body, shape=shape),
        ...
        for _ in range(200): # 2 seconds max
            self.world.update(0.01) # Runs NavigationUpdateSystem (nop) and NavigationSystem (checks results)
        ...
        for node in path:
            ...
            if 175 <= x <= 225:
                ...
                if 60 <= y <= 140:
                     self.fail(f"Path collision detected at {node} inside wall (200,100)")
        
        # Verify it actually moved away from Y=100
        # Either min_y < 50 or max_y > 150
        self.assertTrue(min_y < 60 or max_y > 140, f"Path {path} did not detour significantly. Y-Range: {min_y}-{max_y}")
