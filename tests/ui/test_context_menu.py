...
class TestContextMenu:
    ...
    def context_menu(self, mock_manager):
        ...
        with (
            patch("yukkuri_game.game.ui.context_menu.UIPanel") as MockPanel,
...
class TestContextMenuPositioning:
    ...
    def test_menu_stays_on_screen_right_edge(self, mock_manager):
        ...
        with (
            ...
            rect = call_args.kwargs.get("relative_rect") or call_args[1].get(
                "relative_rect"
            )
...
class TestInventoryIntegration:
    ...
    def test_inventory_view_event_published(self, world):
        ...
        captured_events = []
        event_bus.subscribe(
            InventoryViewRequestedEvent, lambda e: captured_events.append(e)
        )
    ...
    def test_inventory_item_action_event(self, world):
        ...
        captured_events = []
        event_bus.subscribe(
            InventoryItemActionEvent, lambda e: captured_events.append(e)
        )

        event_bus.publish(
            InventoryItemActionEvent(
                entity_id=42, item_type_id="cookie", action="drop", quantity=1
            )
        )
