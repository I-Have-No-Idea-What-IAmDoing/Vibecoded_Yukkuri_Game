...
def test_curve_evaluation():
    ...
    # Linear
    cons = Consideration(name="Test", input_key="val", curve_type="linear", params={"m": 1.0, "b": 0.0})
    assert cons.evaluate_curve(0.0) == 0.0
    assert cons.evaluate_curve(50.0) == 0.5
    assert cons.evaluate_curve(100.0) == 1.0

    # Linear with params
    cons = Consideration(name="Test", input_key="val", curve_type="linear", params={"m": 0.5, "b": 0.2})
    # v = 0.5. result = 0.5 * 0.5 + 0.2 = 0.45
    assert cons.evaluate_curve(50.0) == 0.45

    # Inverse Linear
    cons = Consideration(name="Test", input_key="val", curve_type="inverse_linear", params={})
    assert cons.evaluate_curve(0.0) == 1.0
    assert cons.evaluate_curve(100.0) == 0.0

    # Threshold
    cons = Consideration(name="Test", input_key="val", curve_type="threshold", params={"threshold": 0.5})
    assert cons.evaluate_curve(40.0) == 0.0 # 0.4 < 0.5
    assert cons.evaluate_curve(60.0) == 1.0 # 0.6 >= 0.5

    # Logit (S-curve)
    cons = Consideration(name="Test", input_key="val", curve_type="logit", params={"k": 10.0, "x0": 0.5})
...
def test_utility_calculation_break_early():
    ...
    cons2.score.return_value = 1.0 # Should not be called if optimized, but logic might call it
...
def test_validate_actions_warning():
    ...
    with patch('yukkuri_game.game.ai.behavior.BehaviorRegistry') as mock_registry:
        ...
        mock_actions = {
            "Eat": AIAction(weight=1.0, considerations=[], effects=ActionEffect(type="none", stat_changes={}))
        }
