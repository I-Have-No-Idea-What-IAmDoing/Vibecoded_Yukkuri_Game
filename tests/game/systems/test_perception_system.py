...
from yukkuri_game.game.yukkuri_components import AIState, Blackboard, YukkuriStats, Predator, RelationshipRegistry, RelationshipData
...
class TestPerceptionSystem:
    ...
    def test_update_blackboard_basic(self, world, system):
        ...
        system._update_blackboard(world, entity_id, ai_state, blackboard, trans, current_time=100.0)
    ...
    def test_social_context_predator_prey(self, world, system):
        ...
        def try_get_component_mock(e, c):
            if e == entity_id:
                if c == Predator: return my_predator
                if c == YukkuriStats: return None
            if e == target_id:
                if c == Transform: return target_trans
                if c == YukkuriStats: return target_stats
            return None
        ...
        def robust_get_component(e, c):
            if e == entity_id:
                if c == Predator: return my_predator
                if c == YukkuriStats: return None 
                if c == RelationshipRegistry: return None
            if e == target_id:
                if c == Transform: return target_trans
                if c == YukkuriStats: return target_stats
                if c == Predator: return None
            return None
    ...
    def test_memory_persistence(self, world, system):
        ...
        world.try_get_component.side_effect = lambda e, c: target_trans if (e==target_id and c==Transform) else None
        ...
        system._update_blackboard(world, entity_id, ai_state, blackboard, trans, current_time=101.0)
        ...
        system._update_blackboard(world, entity_id, ai_state, blackboard, trans, current_time=110.0)
