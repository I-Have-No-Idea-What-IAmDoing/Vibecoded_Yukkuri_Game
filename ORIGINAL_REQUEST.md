...
## Follow-up — 2026-06-21T18:19:08Z

Implement Bevy 0.19 native rendering, assets loading, sprite sheets/atlases, animation state mapping, and camera controller systems to advance the Rust/Bevy game engine port.

Working directory: c:\Users\gamin\OneDrive\Documents\Git\Vibecoded_Yukkuri_Game
Integrity mode: development

*Note: The git branch `MVP_python` holds the original Python codebase and can be inspected if needed.*

## Requirements

### R1. Sprites, Atlases, & Prefab Assets
- Parse sprite dimensions (width, height), texture file names, and animation definitions dynamically from configuration files (e.g. `data/yukkuris/types.toml`).
- Load textures and construct Bevy `TextureAtlasLayout` structures on-the-fly using Bevy's `AssetServer` and texture atlas assets.
- Integrate sprite loading with the prefab spawning pipeline (`spawn_yukkuri_prefab` in `src/prefabs/mod.rs`), attaching Bevy `Sprite` and `TextureAtlas` components to the spawned entities so they render visually.
- Support single-frame fallback images when no animations are specified.

### R2. Animation System & FFI Action Mapping
- Implement a Bevy system and components (`Animator` component) that advance animation frames based on delta time (`Time<Virtual>`).
- Support standard looping, ping-pong playback (breath/oscillation effect), speed multipliers, and frame events (triggered when entering specific frames).
- Synchronize animations with the `AIState` (e.g., play animation named after `ai_state.current_action` in lower case) and override with flight states (e.g., "swoop" or "fly" animations when airborne).
- Process `CommandType::PlayAnimation` commands received from Python FFI and map them to the Bevy entity's animator state.

### R3. Camera Controller & Entity Tracking
- Setup a 2D camera system using Bevy's standard camera components.
- Implement smooth interpolation (lerp) toward a tracked entity:
  ```rust
  camera_x += (target_x - camera_x) * 5.0 * dt;
  camera_y += (target_y - camera_y) * 5.0 * dt;
  ```
- Support the following controls:
  - **Panning**: WASD keys and Arrow keys pan the camera. Panning instantly releases the tracking lock.
  - **Middle-mouse Drag Panning**: Mouse drag with middle button pans the camera and releases the tracking lock.
  - **Zooming**: Scroll wheel and Page Up / Page Down keys smoothly zoom the camera (adjusting projection scale).
  - **Left-Click Selection**: Left-clicking an entity selects it and engages tracking lock. Selection must use entity transforms and collision bounds (e.g., Avian circle colliders).
  - **F Key Focus**: Pressing the `F` key re-focuses and locks tracking on the currently selected entity.
- Handle coordinate translation if checking cursor clicks or converting world boundaries.

## Acceptance Criteria

### Component Initialization
- [ ] Yukkuri entities spawned via prefabs have the correct Bevy `Sprite`, `TextureAtlas` (if animated), and `Animator` components attached.
- [ ] Animator component parses and loads `AnimationDefinition` fields (frames, frame_duration, loop, ping_pong, events) correctly from config.

### Animation Runtime & FFI
- [ ] Animation system correctly advances frames, loops, and ping-pongs frames.
- [ ] Animation events are fired in Bevy when specific frames are hit.
- [ ] Receives and executes `CommandType::PlayAnimation` by switching the entity's active animation.
- [ ] Syncs animation with `AIState::current_action` and applies flight state overrides.

### Camera Controller
- [ ] Camera smoothly follows the tracked entity when a tracking lock is active.
- [ ] Keyboard WASD / Arrow movement and Middle-Mouse drag panning instantly release the tracking lock.
- [ ] Scroll wheel and PgUp / PgDn smoothly zoom the camera.
- [ ] Left-clicking within the collider radius of a Yukkuri selects it and locks the camera onto it.
- [ ] Pressing the `F` key re-locks and centers the camera on the selected entity.

### Automated Tests
- [ ] Integration tests added/updated under `tests/` verifying the camera following logic, lock breaking, animation state changes upon commands, and frame updates.
- [ ] All new and existing tests pass successfully when run via the test runner.
